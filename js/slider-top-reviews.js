...
function handleSliderVisibility() {
  ...
  reviews.forEach((review, index) => {
    if (index <= sliderCurrentIndex + 2 && index >= sliderCurrentIndex) {
...
// Function to slide reviews
    function slideReviews(direction) {
      ...
      if (direction === 'right') {
        sliderCurrentIndex = (sliderCurrentIndex + 1) % (reviews.length - 2);
      } else if (direction === 'left') {
        sliderCurrentIndex = (sliderCurrentIndex - 1 + reviews.length - 2) % (reviews.length - 2);
      }
...
const offset = -sliderCurrentIndex * (reviewWidth + gap - 0.35); // Account for gap
...
rightButton.addEventListener('click', () => {
  if (sliderCurrentIndex <= reviews.length - 2) {
    slideReviews('right');
    isPaused = true; // Pause sliding when manually navigating
  }
});
